...
extern unsigned int __VMATextM4;
extern unsigned int __LMATextM4;
extern unsigned int __vectors_start__;
extern unsigned int __resetISR_LMA;
//*****************************************************************************
//
// The vector table.
...
// *************************************************************
    ...
    unsigned int loop;
    /* execute copying to RAM only if program is loaded into FLASH */
    if (&__VMATextM4 != &__LMATextM4) {
        ...
        /* the jump to RAM happens at next function call because all
        linked code for M4 has VMA in RAM space */
    } else {
